...
if __name__ == "__main__":
    ...
    # Configure IterKit
    # ------------------------------
    config.set_project("stroke_prediction_v7")
    config.set_class_names(['0', '1'])
    config.set_norm_order_set(["first", "last"])  # Whether to normalize before augmentation/imbalance
    
 

    config.set_split_ratio_set([0.2,0.1,0.15]) # train/val/test splits

    config.set_prob_threshold_set([0.5,0.3,0.2,0.1])
    ...
    #config.set_models(modelnames=["XGBClassifier_SPW24", "BalancedRandomForest","EasyEnsemble"] )
    config.set_models(modelnames=[ "random_forest", "XGBmodel",  "LogisticRegression","GradientBoosting",  "BalancedRandomForest" ] )
  
    #config.set_models(modelnames=["tf_NeuralNetworkA", "tf_NeuralNetworkB"] )
 
 
    # Optional augmentation/imbalance settings
    config.set_aug_methods([ "gaussian_noise"])
    ...
    #config.set_aug_imbalance_combination(["none", "aug_only"]) # all combinations 0 both none ; 1 aug only; 2 imbalance only; 3 both
 
    config.set_featureNumSet([7,8])
    ...
    config.set_fs_methodset([ "infgain"])
